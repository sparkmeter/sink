defmodule Sink.EventLog.EctoGenericEventLogTest do
  ...
  describe "with no client_id" do
    test "logs an event, can retrieve the same event" do
      assert :ok =
               TestEctoGenericEventLog.log(
                 {@event_type_id, @event_key},
                 @offset,
                 {@event_data, @timestamp}
               )

      assert {@event_data, @timestamp} ==
               TestEctoGenericEventLog.get({@event_type_id, @event_key}, @offset)
    end

    test "gets latest" do
      assert :ok =
               TestEctoGenericEventLog.log({@event_type_id, @event_key}, 1, {<<1>>, @timestamp})

      assert :ok =
               TestEctoGenericEventLog.log({@event_type_id, @event_key}, 2, {<<2>>, @timestamp})

      assert {2, <<2>>, @timestamp} ==
               TestEctoGenericEventLog.get_latest({@event_type_id, @event_key})
    end
  end
  ...
  describe "check_dupe" do
    test "is {:ok, nil} for a record that doesn't exist" do
      assert {:ok, false} =
               TestEctoGenericEventLog.check_dupe(
                 {@event_type_id, @event_key},
                 1,
                 {<<2>>, @timestamp}
               )
